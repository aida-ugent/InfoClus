...
class InfoClus:
    ...
    def __init__(self, dataset_name: str,     # necessary
        ...
        file_path = os.path.join(self.data_obj.cache_path, emb_name + '_' + linkage + '_' + 'modify_' + str(modify_hierarchical))
        if self.allow_cache:
    ...
    def _update_paras(self,  alpha, beta, min_att, max_att, run_id, split_strategy_id):
    ...
    def optimise(self,
                 alpha=None, beta=1.5, min_att=2, max_att=5, run_id=5, split_strategy_id=0,
                 allow_cache=True):
        """
        ...
        self.allow_cache = allow_cache
        self._update_paras(alpha,beta,min_att,max_att,run_id,split_strategy_id)
