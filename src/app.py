...
infoclus_obj = build_infoclus(data_name)
infoc_para_res_dict = infoclus_obj.optimise()
labels = {'InfoClus': infoc_para_res_dict['clustering']}

embeddings_load = np.load(os.path.join(PROJECT_ROOT,'data', data_name, 'embeddings.npz'))
        ...
        dbc.Container(children=
            [
                ...
                dbc.Container(
                    ...
                    className="my-navbar",
                    style={'height': '85vh', 'display': 'flex', 'flexDirection': 'column', 'paddingLeft': '1rem', 'paddingRight': '1rem'}
            )],
            fluid=True,
            style={'height': '100vh'}
        )]
