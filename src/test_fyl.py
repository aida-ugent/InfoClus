...
if DATA_SET_NAME == "cytometry":
    ...
    path_to_emb_file = f'{DATA_FOLDER}/{DATA_SET_NAME}_emb.pkl'
    embedding = from_cache(path_to_emb_file)['Y']
    EMB_NAME = 'Y - from Xander package'
    optimiser = ExclusOptimiser(df_data, df_data_scaled, lenBinary, embedding,
                                alpha=alpha, beta=beta, min_att=min_att, max_att=max_att, runtime_id=runtime_id,
